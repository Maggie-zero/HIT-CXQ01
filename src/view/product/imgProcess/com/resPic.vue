...
const reset = () => {
  if (state.img.file) {
    ...
    imgDraw(
      state.img.transform,
      D3Res.value.offsetWidth / state.img.domWidth,
      D3Res.value.offsetHeight / state.img.domHeight
    );
  }
};
// 标尺
...
// 重新大小
const resize = () => {
  d3.select("#resArea").select("#resCanvas").remove();
  d3.select("#resArea").selectAll("svg").remove();
  nextTick(() => {
    if (D3Res.value) {
      areaInit(D3Res.value.offsetWidth, D3Res.value.offsetHeight);
      gridInit(D3Res.value.offsetWidth, D3Res.value.offsetHeight);
      if (state.img.file) {
        // imgDraw(state.img.file);
...
const watchEl = () => {
  if (true) {
    D3Res.value.style.transition = "width 1s, height 1s";
  }
  const resizeObserver = new ResizeObserver(() => resize());
  resizeObserver.observe(D3Res.value);
};
// 初始化整体空间
          ...
          .join(
            (enter: any) =>
              enter
                .append("line")
                .attr("class", "x")
                .attr("y2", D3Res.value.offsetHeight),
            (update: any) => update,
          ...
          .join(
            (enter: any) =>
              enter
                .append("line")
                .attr("class", "y")
                .attr("x2", D3Res.value.offsetWidth),
            (update: any) => update,
            (exit: any) => exit.remove()
          )
...
const imgInit = (file: any) => {
  ...
  state.img.domWidth = D3Res.value.offsetWidth;
  state.img.domHeight = D3Res.value.offsetHeight;
  // 初始化网格
  // 计算原始比例尺最大高度和宽度
  state.img.lx =
  ...
  state.img.lk =
    Math.min(
      D3Res.value.offsetHeight / state.img.height,
      D3Res.value.offsetWidth / state.img.width
    ) * 0.8;
  gridInit(D3Res.value.offsetWidth, D3Res.value.offsetHeight);
  // 画图
  state.canvas
...
const loadRes = (data: any) => {
  imgInit(data);
};
const removeRes = () => {
  state.img = {} as any;
  resize();
  // d3.select("#area").select("#resCanvas").remove();
  // state.img = {} as any;
  // if (Object.keys(state.img).length > 0) {
  //   d3.select("#area").select("#resCanvas").remove();
  //   state.img = {} as any;
  // }
};

defineExpose({ loadRes, removeRes });
onMounted(() => {
  areaInit(D3Res.value.offsetWidth, D3Res.value.offsetHeight);
  watchEl();
  nextTick(() => {});
